...
export function Web3Modal({ children, className, onClose, ...props }) {
	...
	return (
		<Modal className='bg-base/80' onClose={onClose} {...props}>
			<div>
				<div className='p-8 flex flex-col gap-y-2 bg-primary font-retro rounded-xl border border-secondary'>
					{Wallets.map(({ Icon, connector }) => {
						...
						return (
							<div
								...
								className='w-60 py-2 px-4 text-black rounded border border-gray-400/40 bg-black/20 hover:bg-black/30 hover:outline outline-1 outline-secondary hover:cursor-pointer flex items-center justify-between group'>
